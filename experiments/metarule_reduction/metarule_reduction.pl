...
metarule_variations(I,K,T,L,Ts,Hs,M,S,Acc,Bind):-
        debug(progress,'(~w) Step ~w of ~w',[T,I,K])
...
metarule_subset_evaluations(T,L,[Pos,Neg,BK,MS],Hs,M,S,Vs):-
        ...
        ,debug(progress,'(~w) Random permutation of MS: ~w',[T,MS])
...
metarule_subset_evaluation(T,L,[Pos,Neg,BK,MS],Hs,M,S,N,V):-
        debug(progress,'(~w) User-defined metarules: ~w',[T,MS])
        ,learn_meta_reduction(T,L,[Pos,Neg,BK,MS],Hs,M,S,Ps,V)
        ,debug(progress,'(~w) Measured ~w: ~w',[T,M,V])
        ,debug(metarule_reduction_full,'(~w) Measured ~w: ~w sec',[T,M,V])
        ,debug_clauses(metarule_reduction_full,'Learned:',Ps)
        ,length(Ps, Len)
        ,debug(progress,'(~w) Hypothesis size: ~w',[T,Len])
        ,debug(metarule_reduction_full,'(~w) Hypothesis size: ~w',[T,Len])
        ,length(MS, N).
...
learn_meta_reduction(T,L,[Pos,Neg,BK,MS],Hs,time,S,Ps,D):-
        !
        ,evaluation:train_test_splits(S,Pos,Pos_Train,_Pos_Test)
        ,evaluation:train_test_splits(S,Neg,Neg_Train,_Neg_Test)

        % The output of this will need to be combined with MS below.
        ,debug(progress,'(~w) Learning new metarules specialising: ~w',[T,Hs])
...
combined(MS_1,MS_2,MS):-
        ...
        ,debug_quantified_metarules(metarule_reduction,'All metarules:',MS_)
        ,copy_term(MS_,MS_c)
        ,findall(H-B
                ,(member(H:-B,MS_c)
                 ,numbervars(H:-B)
        ...
        ,findall(H:-B
                ...
                ,MS)
        ,length(MS,N)
        ,debug_quantified_metarules(metarule_reduction,'Combined metarules:',MS)
        ,debug(progress,'Combined metarules: ~w',[N]).
